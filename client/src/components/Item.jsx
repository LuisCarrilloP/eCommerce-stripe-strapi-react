  ...
  return (
    <Box width={width}>
      ...
      <Box position="relative" onMouseOver={() => setIsHovered(true)} onMouseOut={() => setIsHovered(false)}>
        ...
        <img alt={item.name} width="300px" height="400px" src={`http://localhost:1337${item?.attributes?.image?.data?.attributes?.formats.medium.url}`} onClick={() => navigate(`/item/${item.id}`)} style={{ cursor: "pointer" }}/>
